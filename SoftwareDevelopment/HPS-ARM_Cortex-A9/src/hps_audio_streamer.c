...
void hps_stream_init_shared(volatile shared_audio_mem_t *m)
{
    ...
    /* Make the reset ring visible before the poll loop / Nios proceed. */
    __sync_synchronize();
}
...
/* push current song's metadata into shared memory (volatile -> byte copy) */
static void publish_meta(volatile shared_audio_mem_t *m)
{
    ...
    for (unsigned i = 0; i < META_TEXT_MAX; i++) { m->current_meta.title[i]  = t[i];  if (!t[i])  break; }
    for (unsigned i = 0; i < META_TEXT_MAX; i++) { m->current_meta.artist[i] = a[i];  if (!a[i])  break; }
    for (unsigned i = 0; i < META_TEXT_MAX; i++) { m->current_meta.album[i]  = al[i]; if (!al[i]) break; }

    m->current_meta.duration_seconds = g_wav.duration_seconds;
    m->current_meta.sample_rate      = g_wav.sample_rate;
}

/* open song `idx`, reset the ring, and prime-fill every buffer from the start */
static int load_song(volatile shared_audio_mem_t *m, unsigned idx)
{
    ...
    if (wav_open(g_paths[idx], &g_wav) != 0) {
        fprintf(stderr, "[HPS] failed to open %s\n", g_paths[idx]);
        return -1;
    }
    g_wav_open  = 1;
    g_cur_song  = idx;
    ...
    /* prime-fill every buffer (initial / new song) */
    for (unsigned i = 0; i < NUM_BUFFERS; i++)
...
void hps_stream_try_fill_next_buffer(volatile shared_audio_mem_t *m)
{
    ...
    /* RAW copy: wav_read_pcm_chunk freads straight into shared mem, no
     * transformation -- per the hard requirement. */
    ...
    /* PUBLISH BARRIER. On the weakly-ordered ARM, the payload + size + flags
     * stores above must be globally visible BEFORE the READY store below;
     * otherwise the Nios can observe READY first and consume a half-written
     * buffer, desyncing the lock-free ring (the "plays ~1s then wedges" bug). */
    __sync_synchronize();
...
void hps_stream_handle_nios_events(volatile shared_audio_mem_t *m)
{
    ...
    /* BUF_EMPTY no longer uses the event channel -- buffers are refilled by
     * the round-robin top-up pass based on state alone. */

    /* PUBLISH BARRIER. All buffer/state/metadata writes from load_song must be
     * visible before we echo the ack the Nios is busy-waiting on. */
    __sync_synchronize();
