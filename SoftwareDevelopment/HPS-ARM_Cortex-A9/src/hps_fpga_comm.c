...
static void bus_handler(int sig) { (void)sig; g_bus_fault = 1; siglongjmp(g_bus_jmp, 1); }
...
int hps_fpga_init(uint32_t phys_addr, uint32_t size)
{
    ...
    for (unsigned attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        printf("[HPS] mmap failed (attempt %u/%u), retrying in 0.5s...\n",
               attempt + 1, MAX_RETRIES);
        usleep(RETRY_SLEEP_US);
    }
    fprintf(stderr, "[HPS] bridge never came up after %u attempts, giving up\n", MAX_RETRIES);
    return -1;
}
