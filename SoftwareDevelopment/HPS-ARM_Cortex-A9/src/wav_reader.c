                               ...
                               const char *src, uint32_t src_size)
{
    ...
    /*
     * Some WAV metadata strings include a trailing '\0'.
     * This loop also removes simple trailing spaces/newlines.
     */
    while (copy_size > 0u &&
           (dst[copy_size - 1u] == '\0' ||
            dst[copy_size - 1u] == ' '  ||
...
static int parse_info_metadata_chunk(FILE *file,
                                     uint32_t list_content_size,
                                     wav_metadata_t *metadata)
{
    ...
    /*
     * A LIST chunk starts with a 4-byte list type.
     * For WAV metadata, we care about LIST type "INFO".
     */
    if (read_exact(file, list_type, sizeof(list_type)) != 0) {
        return -1;
    }

    if (memcmp(list_type, "INFO", 4) != 0) {
        /*
         * Not an INFO metadata list.
         * Skip the rest of the LIST content.
         */
        return skip_bytes(file, list_content_size - 4u);
    }
    ...
    while (consumed + 8u <= list_content_size) {
        ...
        /*
         * Only read metadata values that fit into our temporary buffer.
         * If the value is larger, read only what we can use and skip the rest.
         */
        if (info_size > 0u) {
            uint32_t read_size = info_size;

            if (read_size >= META_TEXT_MAX) {
                read_size = META_TEXT_MAX - 1u;
            }

            if (read_exact(file, temp, read_size) != 0) {
                return -1;
            }
            ...
            if (info_size > read_size) {
                if (skip_bytes(file, info_size - read_size) != 0) {
                    return -1;
                }
            }

            if (memcmp(info_id, "INAM", 4) == 0) {
                copy_metadata_text(metadata->title,
                                   META_TEXT_MAX,
                                   temp,
                                   read_size);
            } else if (memcmp(info_id, "IART", 4) == 0) {
                copy_metadata_text(metadata->artist,
                                   META_TEXT_MAX,
                                   temp,
                                   read_size);
            } else if (memcmp(info_id, "IPRD", 4) == 0 ||
                       memcmp(info_id, "IALB", 4) == 0) {
                copy_metadata_text(metadata->album,
                                   META_TEXT_MAX,
                                   temp,
                                   read_size);
            }
        ...
        /*
         * INFO subchunks are also padded to even byte boundaries.
         */
        if (info_size & 1u) {
            if (fseek(file, 1L, SEEK_CUR) != 0) {
                return -1;
    ...
    /*
     * If there are leftover bytes in the LIST chunk, skip them.
     */
    ...
    if (consumed < list_content_size) {
        if (skip_bytes(file, list_content_size - consumed) != 0) {
            return -1;
        }
    }
...
int wav_open(const char *path, wav_info_t *info)
{
     ...
     * Scan the WAV chunks.
     *
     * We do not stop permanently at "data" because metadata may appear
     * before or after it. Instead, we record the data offset and continue.
     */
    while (read_exact(info->file, chunk_id, sizeof(chunk_id)) == 0 &&
        ...
        if (memcmp(chunk_id, "fmt ", 4) == 0) {
            ...
            if (parse_info_metadata_chunk(info->file,
                                          chunk_size,
                                          &info->metadata) != 0) {
                wav_close(info);
                return -1;
            ...
            /*
             * Skip data for now so we can continue scanning possible metadata.
             * At the end, we seek back to data_offset.
             */
    ...
    /*
     * If metadata is missing, use safe fallback values.
     */
    if (info->metadata.title[0] == '\0') {
        copy_metadata_text(info->metadata.title,
                           META_TEXT_MAX,
                           "Unknown Title",
                           13u);
    }

    if (info->metadata.artist[0] == '\0') {
        copy_metadata_text(info->metadata.artist,
                           META_TEXT_MAX,
                           "Unknown Artist",
                           14u);
    }

    if (info->metadata.album[0] == '\0') {
        copy_metadata_text(info->metadata.album,
                           META_TEXT_MAX,
                           "Unknown Album",
                           13u);
    }
    ...
    /*
     * Seek back to the start of raw PCM data.
     * wav_read_pcm_chunk() will continue from here.
     */
                       ...
                       int *is_last)
{
    ...
    if (info == NULL || info->file == NULL ||
        dst == NULL || bytes_read == NULL || is_last == NULL) {
        return -1;
    }
    ...
    *is_last = 0;
    ...
    if (to_read > info->bytes_remaining) {
        to_read = info->bytes_remaining;
    }

    if (fread((void *)dst, 1, to_read, info->file) != to_read) {
        return -1;
    }
